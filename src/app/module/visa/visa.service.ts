...
const createVisa = async (payload: TVisa) => {
  const isExistVisa = await Visa.findOne({
    passportNumber: payload?.passportNumber,
  });
  if (isExistVisa) {
    throw new AppError(
      StatusCodes.CONFLICT,
      "Visa already exists with this passport number!"
    );
  }
...
const getAllVisa = async (query: Record<string, unknown>) => {
  ...
  const totalVisas = await Visa.countDocuments(
    visaQuery.queryModel.getFilter()
  );
...
const updateVisa = async (id: string, updatedVisaData: Partial<TVisa>) => {
  const result = await Visa.findByIdAndUpdate(id, updatedVisaData, {
    new: true,
  });
  return result;
};
